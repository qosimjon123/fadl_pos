...
def _get_scale_barcode_settings():
	...
	try:
		...
		frappe.log_error("Unable to load Scale Barcode Settings", "Fadl Pos")
		return None
...
def _find_item_scale_template(item_code: str, uom: str | None = None) -> str:
	...
	ordered_rows = rows
	if requested_uom:
		matched = [row for row in rows if cstr(row.get("uom") or "").strip() == requested_uom]
		unmatched = [row for row in rows if cstr(row.get("uom") or "").strip() != requested_uom]
...
def build_scale_barcode(
	barcode_template: str | None = None,
