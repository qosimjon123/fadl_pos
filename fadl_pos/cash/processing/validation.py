...
from fadl_pos.payment.processing.utils import get_bank_cash_account
...
def get_opening_entry(opening_entry_name):
	...
	if opening_entry.docstatus != 1 or opening_entry.status != "Open":
		frappe.throw(_("POS Opening Entry must be submitted and open."))
...
def validate_company_consistency(opening_entry, profile_doc):
	if opening_entry.pos_profile != profile_doc.name:
		frappe.throw(_("POS Profile must match the active POS Opening Entry profile."))
	if opening_entry.company != profile_doc.company:
		frappe.throw(_("POS Profile company must match active POS Opening Entry company."))
...
def resolve_target_account(payload, profile_doc, movement_type):
	...
	if movement_type == "Expense":
		account = (
			payload.get("expense_account") or profile_doc.get("default_pos_expense_account") or ""
		).strip()
