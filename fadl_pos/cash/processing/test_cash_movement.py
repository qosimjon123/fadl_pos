...
from fadl_pos.cash.processing import queries, service, validation


class TestCashMovementValidation(unittest.TestCase):
	@patch("fadl_pos.cash.processing.validation.frappe")
...
class TestCashMovementService(unittest.TestCase):
	@patch("fadl_pos.cash.processing.service.parse_payload")
	@patch("fadl_pos.cash.processing.service.get_opening_entry")
	@patch("fadl_pos.cash.processing.service.get_pos_profile")
	@patch("fadl_pos.cash.processing.service.validate_company_consistency")
	@patch("fadl_pos.cash.processing.service.ensure_feature_enabled")
	@patch("fadl_pos.cash.processing.service.ensure_movement_allowed")
	@patch("fadl_pos.cash.processing.service.validate_amount")
	@patch("fadl_pos.cash.processing.service.validate_remarks")
	@patch("fadl_pos.cash.processing.service.ensure_no_duplicate_client_request")
	@patch("fadl_pos.cash.processing.service.resolve_source_cash_account")
	@patch("fadl_pos.cash.processing.service.resolve_target_account")
	@patch("fadl_pos.cash.processing.service.validate_account_company")
	@patch("fadl_pos.cash.processing.service.create_journal_entry")
	@patch("fadl_pos.cash.processing.service.frappe")
	def test_create_cash_movement_returns_existing_when_client_request_replayed(
		...
		mock_get_opening_entry,
		mock_parse_payload,
	):
		opening_entry = SimpleNamespace(name="POS-OPEN-1", pos_profile="POS-PROFILE-1")
		...
		mock_parse_payload.return_value = {"pos_opening_entry": "POS-OPEN-1", "client_request_id": "req-1"}
		mock_get_opening_entry.return_value = opening_entry
	...
	@patch("fadl_pos.cash.processing.service._create_cash_movement")
	@patch("fadl_pos.cash.processing.service.ensure_owner_or_manager")
	@patch("fadl_pos.cash.processing.service.frappe")
...
class TestCashMovementQueries(unittest.TestCase):
	@patch("fadl_pos.cash.processing.queries.frappe.get_all")
	def test_get_shift_movements_maps_status_to_docstatus(self, mock_get_all):
		...
		self.assertEqual(kwargs["filters"]["pos_opening_entry"], "POS-OPEN-1")
