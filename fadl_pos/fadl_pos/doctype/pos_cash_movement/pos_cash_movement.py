...
from fadl_pos.cash.processing.posting import cancel_journal_entry
...
class POSCashMovement(Document):
	...
	def _validate_company_links(self):
		...
		if opening_shift.company != self.company:
			frappe.throw(_("Company must match the selected POS Opening Entry company."))

		if opening_shift.pos_profile != self.pos_profile:
			frappe.throw(_("POS Profile must match the selected POS Opening Entry profile."))

		if opening_shift.user != self.user:
			frappe.throw(_("User must match the selected POS Opening Entry user."))
