...
from fadl_pos.payment.processing.data import get_outstanding_invoices, get_unallocated_payments
	...
	for payment in unallocated_payments:
		...
		if cint(payment.get("is_credit_note")) or payment.get("voucher_type") == "Sales Invoice":
			...
			try:
				...
				frappe.log_error(
					...
					message=f"Failed to auto reconcile credit note {payment_name}: {exc!s}",
				)
				continue
		...
		if payment.get("voucher_type") == "Journal Entry":
			...
			try:
				...
				frappe.log_error(
					title="POS Auto Reconcile Error",
					message=f"Failed to auto reconcile journal entry {payment_name}: {exc!s}",
				)
				continue
		...
		try:
			...
			frappe.log_error(
				title="POS Auto Reconcile Error",
				message=f"Failed to auto reconcile payment {payment_name}: {exc!s}",
			)
			continue
