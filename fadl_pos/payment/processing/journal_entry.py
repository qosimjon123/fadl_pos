...
from fadl_pos.payment.processing.utils import get_bank_cash_account, get_party_account
...
def create_direct_journal_entry(
	...
	try:
		...
		frappe.log_error(f"Error creating direct journal entry: {e!s}", "Direct JE Error")
		frappe.throw(f"Failed to create journal entry: {e!s}")
